...
function UserList({ setIsOpenModal }: { setIsOpenModal: any }) {
  ...
  const reloadData = async () => {
    ...
    const responseData = await axiosApi.get(
      "http://localhost:8080/api/user/" + userId + "/friend"
    );
    ...
    const responseUserData = await axiosApi.get("http://localhost:8080/api/user");
    ...
    for (let i = 0; i < useridx; i++) {
      if (response[i].id != 0) {
        const responseDetail = await axiosApi.get(
          "http://localhost:8080/api/user/" + response[i].id
        );
        const detailResponse = responseDetail.data;
        const responseMatch = await axiosApi.get(
          "http://localhost:8080/api/user/" + response[i].id + "/matchhistory"
        );
  ...
  async function follow(index: number) {
    const apiUrl = "http://localhost:8080/api/user/" + userId + "/friend";
    const dataToUpdate = {
      isAdd: true,
